...
paraphrase_model_name_or_path="tdopierre/ProtAugment-ParaphraseGenerator"

gpu_id=1
for seed in 42 330; do
    for cv in 01 02 03; do
        for C in 5; do
            for K in 5 1; do
                for aug_type in "C_only" "E_only"; do 
                    ...
                    for dataset in BANKING77; do
                        ...
                        OUTPUT_ROOT="runs/10samp/${dataset}/${cv}/${C}C_${K}K/seed${seed}/paraphrase-td_${cv}"
                        ...
                        model_params="
                            ...
                            --model-name-or-path tdopierre/ProtAugment-LM-${dataset}"
                        ...
                        # '---------------------------------------------'
                        OUT_PATH="runs/full_datasets/${dataset}/${cv}/${C}C_${K}K/seed${seed}/proto-euclidean_${aug_type}"
                        if [[ -d "${OUT_PATH}" ]]; then
                            echo "${OUT_PATH} already exists. Skipping."
                        else
                            mkdir -p ${OUT_PATH}
                            ...
                            models/proto/protonet_modify.sh \
                                --train-path data/${dataset}/few_shot/${cv}/${aug_type}_train_.jsonl \
                                --valid-path data/${dataset}/few_shot/${cv}/valid.jsonl \
                                --test-path data/${dataset}/few_shot/${cv}/test.jsonl \
                                --train-aug=1 \
                                --model-name-or-path "tdopierre/ProtAugment-LM-${dataset}" \
